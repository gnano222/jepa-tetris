...
def test_predictor_hierarchical_film_attn_shape():
    pred = Predictor(patch_dim=128, num_patches=N_DEFAULT, hierarchical_film_attn=True)
    z = torch.randn(8, N_DEFAULT, 128)
    a = torch.randn(8, 128)
    out = pred(z, a)
    assert out.shape == (8, N_DEFAULT, 128)
...
def test_predictor_hierarchical_film_attn_pos_emb_shape():
    pred = Predictor(patch_dim=128, num_patches=N_DEFAULT, hierarchical_film_attn=True)
    assert pred.pos_emb.shape == (1, N_DEFAULT, 128)


def test_predictor_hierarchical_film_attn_mutually_exclusive():
    with pytest.raises(ValueError, match="mutually exclusive"):
        Predictor(patch_dim=128, num_patches=N_DEFAULT, hierarchical_film_attn=True, film=True)
